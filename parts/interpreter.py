...
class TfLite(Interpreter):
    ...
    def load(self, model_path):
        ...
        logger.info(f'Loading model {model_path}')
        # Load TFLite model and extract input and output details.
        # Signatures path bypassed for compatibility with models converted
        # under older TF versions that do not emit SignatureDefs.
        Interpreter = get_tflite_interpreter()
        self.interpreter = Interpreter(model_path=model_path)
        self.interpreter.allocate_tensors()
        self.input_details = self.interpreter.get_input_details()
        self.output_details = self.interpreter.get_output_details()
        # DonkeyCar uses a single input named 'img_in' by convention
        self.input_keys = ['img_in']
        # Output keys are what predict_from_dict returns; order matches output_details
        self.output_keys = [d['name'] for d in self.output_details]

    def compile(self, **kwargs):
        pass

    def predict_from_dict(self, input_dict):
        # Set each input tensor by matching order with input_details
        for i, k in enumerate(self.input_keys):
            arr = self.expand_and_convert(input_dict[k])
            self.interpreter.set_tensor(self.input_details[i]['index'], arr)
        self.interpreter.invoke()
        # Read outputs in the order they appear in output_details
        outputs = [self.interpreter.get_tensor(d['index'])[0] for d in self.output_details]
        return outputs if len(outputs) > 1 else outputs[0]

    def get_input_shape(self, input_name):
        assert self.interpreter is not None, "Need to load tflite model first"
        # No signatures to look up; return the first (and typically only) input's shape
